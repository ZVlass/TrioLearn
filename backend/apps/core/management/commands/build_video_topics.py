...
def main():
    # Load the .env nearest to current working tree; override any stale OS env
    ...
    videos_csv = need_path("VIDEOS_CSV")
    ...
    if not videos_csv.exists():
        raise FileNotFoundError(f"VIDEOS_CSV not found: {videos_csv}")
    ...
    # Select likely text columns
    text_cols_pref = [
        "title", "description", "tags", "channel", "subtitle", "summary", "transcript"
    ]
    text_cols = [c for c in text_cols_pref if c in df.columns]
    if not text_cols:
        raise RuntimeError("No text columns found in videos CSV")
    ...
    if df.empty:
        raise RuntimeError("No video rows after cleaning")

    # Vectorize + LDA (use 'english' to satisfy sklearn validator)
    ...
    # Output: one file with per-video topic distribution
    topic_cols = [f"topic_{i:02d}" for i in range(K)]
    out = pd.DataFrame(theta, columns=topic_cols)
    out.insert(0, "id", df["id"].to_numpy())

    try:
        out_path = outdir / "video_topic_vectors.parquet"
        out.to_parquet(out_path, index=False)
    except Exception:
        out_path = outdir / "video_topic_vectors.csv"
